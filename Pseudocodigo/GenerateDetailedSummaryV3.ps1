$outputPath = "Pseudocodigo\Resumen_Estructurado_v3.md"
"# Resumen Estructurado de Pseudocodigo (v3)" | Out-File -FilePath $outputPath -Encoding UTF8

Get-ChildItem -Path Pseudocodigo -Filter *.txt | ForEach-Object {
    $file = $_
    $content = Get-Content -LiteralPath $file.FullName
    $text = $content -join "`n"
    # extract Accion name (case-insensitive, allow optional ES;)
